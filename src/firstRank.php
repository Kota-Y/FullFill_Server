<?php
    ...
    try {
		...
		$stmt = $conn->prepare("SELECT kotoba.kotoba,kotoba.whose,user.username,user.image,kotoba.id FROM kotoba,user WHERE 1 AND kotoba.userid=user.id AND kotoba.id=$kotobaid");
		$stmt->execute();
